...
### Fixed
- **`pi-code-gui.defaultApproval` had no effect.** It was read only to draw the ★ in the approval picker and written when you chose "save as default" — nothing ever applied it, so a new session started in whatever `~/.pi/agent/settings.json` happened to hold. With the default set to `write`, a fresh session could open in `yolo`, the picker showing ★ beside a ✓ on a different row. The setting is now applied at session start, and its default changed from `always-ask` to **empty**, meaning *follow the shared file* — the extension imposes nothing on the file the `pi` CLI also writes unless you ask it to.
