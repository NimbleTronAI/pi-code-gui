...
#
#   (1) The agent config (.claude/settings.json for Claude Code) carries
#       permissions.deny Read() rules for every restricted dependency-source path,
#       so a config edit that drops a rule fails the commit.
#   (2) No live repo file references those dependency-source paths (a script or
#       test that reads the checkout would route the source into logs/context).
...
# file may reference the on-disk source path that matches CARGO_PATH_RE.
...
# the pi_agent_rust cargo git checkout + the asupersync / franken-* registry sources.
CARGO_PATH_RE='\.cargo/(git/checkouts/pi_agent_rust|registry/src/[^"'"'"' ]*/(asupersync|franken))'
...
# logs, and this gate + its smoke — they legitimately name the paths).
SCAN_ROOTS=(crates src scripts frontend docs/wiki AGENTS.md CLAUDE.md)
EXEMPT_GLOBS=(
  '!docs/wiki/log.md'
  '!docs/wiki/archive/**'
...
matches=$(rg -n "$CARGO_PATH_RE" "${SCAN_ROOTS[@]}" "${exempt_args[@]}" 2>&1)
rc=$?
if [[ $rc -eq 2 ]]; then
